"""Chat-LLM provayderlari uchun umumiy asos (OpenAI, Gemini ...).

Provayder faqat `_complete()` ni amalga oshiradi (xabarlar -> matn). Baholash,
hujjat va hisobot mantiqi shu yerda; javob buzilsa yoki server xato bersa
oflayn `MockLLMAdapter` ga qaytiladi.
"""
...
log = logging.getLogger("bahola.llm")
...
class ChatLLMAdapter:
    ...
    # ---- umumiy yordamchilar --------------------------------------------
    async def _chat_json(self, messages: List[dict], *, temperature: float = 0.1, max_tokens: int = 2500) -> Optional[dict]:
        text = await self._complete(messages, temperature=temperature, max_tokens=max_tokens, json_mode=True)
        parsed = extract_json(text)
        if text and parsed is None:
            self.last_error = "JSON parse error"
        return parsed

    async def chat(self, messages: List[dict], *, system: Optional[str] = None, temperature: float = 0.4, max_tokens: int = 900) -> str:
        """Erkin matnli suhbat (yordamchi chat). Xato bo'lsa oflayn FAQ."""
        msgs = ([{"role": "system", "content": system}] if system else []) + list(messages)
        text = await self._complete(msgs, temperature=temperature, max_tokens=max_tokens, json_mode=False)
        if text and text.strip():
            return text.strip()
        return await self.fallback.chat(messages, system=system)
    ...
    async def grade_submission(
        ...
        *,
        ...
        parsed = await self._chat_json([{"role": "system", "content": system}, {"role": "user", "content": "\n\n".join(user_parts)}])
        ...
        if not parsed or not isinstance(parsed.get("rubric"), list):
            heuristic["provider"] = "mock-fallback"
            heuristic["fallback_reason"] = self.last_error
            return heuristic
        ...
        for item in rubric:
            ...
            if src and src.get("score") is not None:
                ...
                evidence = str(src.get("evidence") or h.evidence or "")[:1000]
            else:
        ...
        hf = heuristic["feedback"]
        feedback = {
            "summary": str(fb.get("summary") or hf["summary"]),
            "strengths": [str(x) for x in (fb.get("strengths") or hf["strengths"])][:6],
            "errors": [str(x) for x in (fb.get("errors") or hf["errors"])][:8],
            "suggestions": [str(x) for x in (fb.get("suggestions") or hf["suggestions"])][:6],
        }
        integrity = analyze_text(text, compare_with=compare_with)
        return {
            "total_score": total, "max_score": max_total, "rubric": scored, "feedback": feedback,
            "plagiarism_score": integrity["plagiarism_score"], "ai_likelihood": integrity["ai_likelihood"], "integrity": integrity,
            "features": text_features(text, reference_answer=reference_answer, topic=topic), "provider": self.name, "model": self.model,
        }
    ...
    async def generate_docs(
        ...
        *,
        ...
        if not topic_list:
            ...
            user = (f"«{course_name}» fani uchun {weeks} haftalik o'quv rejasi mavzularini ({weeks} ta, mantiqiy ketma-ketlikda, "
                    f"{level or 'bakalavriat'} darajasi, o'zbek tilida) tuzing. Har bir mavzu qisqa va aniq bo'lsin.")
            parsed = await self._chat_json([{"role": "system", "content": system}, {"role": "user", "content": user}], max_tokens=1500)
        ...
        if "test_questions" in doc_types:
            system = ('Siz test tuzuvchi metodistsiz. Faqat JSON qaytaring: '
                      '{"questions":[{"topic":str,"question":str,"options":{"A":str,"B":str,"C":str,"D":str},"answer":"A|B|C|D"}]}')
            user = (f"«{course_name}» fani, mavzular: {json.dumps(topic_list[:weeks], ensure_ascii=False)}. "
                    f"{n_questions} ta 4 variantli test savoli tuzing (o'zbek tilida, har xil mavzulardan, bitta to'g'ri javob).")
            parsed = await self._chat_json([{"role": "system", "content": system}, {"role": "user", "content": user}], max_tokens=3500)
            questions = []
            if parsed and isinstance(parsed.get("questions"), list):
                for i, q in enumerate(parsed["questions"][:n_questions], start=1):
                    ...
                    questions.append({"number": i, "topic": str(q.get("topic") or ""), "question": str(q.get("question") or ""),
                                      "options": {k: str(v) for k, v in q["options"].items()}, "answer": ans if ans in {"A", "B", "C", "D"} else "A"})
    ...
    async def generate_report_summary(self, analytics: Dict[str, Any], course: Optional[str], group: Optional[str]) -> str:
        ...
        keys = ("total_students", "total_grades", "average_score", "pass_rate", "quality_rate", "distribution", "weak_topics", "criteria_stats", "at_risk_students")
        user = (f"Fan: {course or 'barcha'}; guruh: {group or 'barcha'}. Statistik ma'lumotlar: "
                f"{json.dumps({k: analytics.get(k) for k in keys}, ensure_ascii=False)[:6000]}\n\n"
                "Shu ma'lumotlar asosida 3–4 abzatsli rasmiy tahliliy xulosa (o'zbek tilida) yozing: umumiy holat, qiyin mavzular va sabablari, xavf guruhidagi talabalar, aniq tavsiyalar.")
        parsed = await self._chat_json([{"role": "system", "content": system}, {"role": "user", "content": user}], max_tokens=1200)
